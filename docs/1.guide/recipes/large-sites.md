...
Scan websites with thousands of pages efficiently. Unlike single-page tools like PageSpeed Insights, Kumphouse handles large sites with smart sampling, parallel scanning, and configurable limits.
...
Kumphouse includes smart defaults for large sites. Understanding these helps balance completeness with performance.
...
You can configure Kumphouse to use an explicit list of relative paths. This can be useful if you have a fairly complex
and large site.
...
To make the most intelligent sampling decisions, Kumphouse needs to know which page files are available. When running
using the
integration API, Kumphouse will automatically provide this information.
