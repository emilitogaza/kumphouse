...
Unlike single-page Lighthouse reports, Kumphouse aggregates results across your entire site.
...
Install the CLI globally to use `kumphouse-ci`:
...
```bash
kumphouse-ci --site <your-site> --build-static
```

This will generate files in your `outputPath` (`.kumphouse` by default). You can upload the `client` directory to a static host from there.

If you want to preview the static report you can run `npx sirv-cli .kumphouse/client`
...
```bash
kumphouse-ci --site www.example.com --build-static && wrangler pages publish .kumphouse
```
...
```bash
kumphouse-ci --site <your-site> --reporter csv
```
...
```bash
kumphouse-ci --site <your-site> --reporter json
```
...
```bash
kumphouse-ci --site <your-site> --reporter lighthouseServer --lhci-host <lhci host> --lhci-build-token <lhci buildToken>
```
...
```bash
kumphouse-ci --site <your-site> --reporter lighthouseServer --lhci-host <lhci host> --lhci-build-token <lhci buildToken> --lhci-auth <username>:<password>
```
