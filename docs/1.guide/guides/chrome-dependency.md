...
description: "Configure Chrome browser settings for Kumphouse scanning, including system Chrome usage and custom installations."
keywords:
  - lighthouse chrome
  - puppeteer chrome
  - chromium lighthouse
...
Kumphouse uses your system Chrome installation to keep package size minimal. When Chrome isn't available, it automatically downloads a compatible Chromium binary.

## Disabling system Chrome

You can disable the system chrome usage by modifying the `chrome.useSystem` flag.

This will make Kumphouse download and use the latest Chrome binary instead.
...
- `chrome.downloadFallbackCacheDir` - Where the binary should be saved (default `$home/.kumphouse`)
...
export default defineKumphouseConfig({
  chrome: {
    ...
    downloadFallbackCacheDir: '/tmp/kumphouse',
  },
})
```
