---
title: "Configuration"
description: "Configure Kumphouse for your specific needs using configuration files and inline options."
keywords:
  - kumphouse config
...
Now run `kumphouse` without flags. It picks up your config automatically.
...
Kumphouse looks for these files in order:
- `kumphouse.config.ts`
- `kumphouse.config.js`
- `kumphouse.config.mjs`

Or specify one explicitly:
```bash
kumphouse --config-file ./configs/production.config.ts
```
