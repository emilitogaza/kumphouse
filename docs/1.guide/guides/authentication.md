...
description: "Scan password-protected websites with Kumphouse. Configure basic auth, cookies, headers, localStorage, and programmatic login flows."
keywords:
...
Need to scan pages behind a login? Kumphouse supports every common auth pattern. Find yours below.
...
Or via CLI:
```bash
kumphouse --site staging.example.com --auth admin:secretpass
```
...
CLI shorthand:
```bash
kumphouse --site example.com --cookies "session_id=abc123"

# Multiple cookies
kumphouse --site example.com --cookies "session_id=abc123;csrf_token=xyz789"
```
...
CLI:
```bash
kumphouse --site api.example.com --extra-headers "Authorization:Bearer abc123"
```
...
Kumphouse sets these before each page loads.
...
export default defineKumphouseConfig({
  puppeteerOptions: {
    userDataDir: './.kumphouse-session', // Persist browser data
  },
  lighthouseOptions: {
