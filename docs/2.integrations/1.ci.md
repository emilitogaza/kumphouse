---
title: "kumphouse-ci: Bulk Lighthouse CI for Entire Sites"
icon: i-carbon-terminal-3270
description: "Run kumphouse-ci on every deploy. Auto-discovers every URL, runs Lighthouse on all of them in parallel, enforces a single budget. Works in GitHub Actions, GitLab CI, any pipeline."
keywords:
  - kumphouse-ci
  - kumphouse ci
  - kumphouse github actions
  - kumphouse gitlab ci
  - bulk lighthouse ci
...
## Run `kumphouse-ci` in your pipeline

The `kumphouse-ci` binary runs Lighthouse on every page of your site and fails your CI build if any score drops below a budget:

```bash
npm install -g kumphouse-cli puppeteer
kumphouse-ci --site https://staging.example.com --budget 80
```
...
kumphouse-ci --site example.com --budget 75

# Generate static HTML report
kumphouse-ci --site example.com --build-static

# Both: budget check + report
kumphouse-ci --site example.com --budget 75 --build-static
```

## Already Using LHCI Server?

Keep your existing infrastructure—Kumphouse uploads to LHCI servers:

```bash
kumphouse-ci --site example.com \
...
kumphouse-ci --site example.com --reporter csvExpanded
```
...
Output goes to `.kumphouse/`. Upload that folder to any static host.
...
If you want to configure Kumphouse, you can create a `kumphouse.config.ts` file in your cwd.
  ...
  demo:
    ...
    steps:
      ...
      - name: Install Dependencies
        run: npm install -g kumphouse-cli puppeteer netlify-cli

      - name: Kumphouse assertions and client
        run: kumphouse-ci --site <your-site> --budget 75 --build-static

      - name: Deploy
        run: netlify deploy --dir=.kumphouse --prod --message="New Release Deploy from GitHub Actions"
        env:
...
lighthouse-audit:
  ...
  script:
    - npm install -g kumphouse-cli puppeteer
    - kumphouse-ci --site $SITE_URL --budget 75
  artifacts:
    paths:
      - .kumphouse/
    expire_in: 1 week
...
jobs:
  lighthouse:
    ...
    steps:
      - checkout
      - run: npm install -g kumphouse-cli
      - run: kumphouse-ci --site $SITE_URL --budget 75 --build-static
      - store_artifacts:
          path: .kumphouse
```
