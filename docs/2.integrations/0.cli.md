...
Scan your entire website from the command line with a rich interactive dashboard. Unlike the standard `lighthouse` CLI which tests one page at a time, Kumphouse automatically discovers and audits all your pages.
...
```bash
npx kumphouse --site example.com
```

Kumphouse wraps the [Lighthouse npm package](https://www.npmjs.com/package/lighthouse) to enable site-wide scanning.

## Kumphouse CLI vs Lighthouse CLI

| Feature | lighthouse CLI | kumphouse CLI |
|---------|---------------|------------------|
| Package | `lighthouse` | `kumphouse-cli` |
...
Once installed globally you'll have access to Kumphouse through the `kumphouse` binary.

Run the default scan.

```bash
kumphouse --site example.com --debug
```

Run without caching, throttle the requests and do 3 samples.

```bash
kumphouse --site example.com --debug --no-cache --throttle --samples 3
```
...
| `--urls <urls>`              | Specify explicit relative paths to scan as a comma-separated list, disabling the link crawler.<br>e.g. `kumphouse --site unlighthouse.dev --urls /guide,/api,/config`    |
| `--exclude-urls <urls>`      | Relative paths (string or regex) to exclude as a comma-separated list. <br>e.g. `kumphouse --site unlighthouse.dev --exclude-urls /guide/.*,/api/.*`                     |
| `--include-urls <urls>`      | Relative paths (string or regex) to include as a comma-separated list. <br>e.g. `kumphouse --site unlighthouse.dev --include-urls /guide/.*`                             |
