...
async function run() {
  ...
  const { resolvedConfig, setCiContext, hooks, worker, start } = useKumphouse()
  ...
  hooks.hook('worker-finished', async () => {
    ...
    logger.success(`Kumphouse has finished scanning \`${resolvedConfig.site}\`: ${worker.reports().length} routes in \`${seconds}s\`.`)
