...
import { createKumphouseWorker, inspectHtmlTask, runLighthouseTask } from './puppeteer'
...
const engineContext = createContext<KumphouseContext>()

export { useLogger } from './logger'
/**
 * Use the kumphouse instance.
 */
export const useKumphouse = engineContext.tryUse as () => KumphouseContext

/**
 * A simple define wrapper to provide typings to config definitions.
 * @deprecated Use `defineKumphouseConfig` from `kumphouse/config` instead.
 */
...
/**
 * Create a unique single kumphouse instance that can be referenced globally with `useKumphouse()`. Scanning will
 * not start automatically, a server context needs to be provided using `setServerContext()`.
 *
 * @param userConfig
 * @param provider
 */
export async function createKumphouse(userConfig: UserConfig, provider?: Provider) {
  ...
  logger.debug(`Starting Kumphouse at root: \`${userConfig.root}\` cwd: ${process.cwd()}`)
  ...
  ;(globalThis as any).defineKumphouseConfig = (c: any) => c
  const { configFile, config } = await loadConfig<UserConfig>({
    name: 'kumphouse',
    cwd: userConfig.root,
    configFile: userConfig.configFile || 'kumphouse.config',
    dotenv: true,
  })
  delete (globalThis as any).defineKumphouseConfig
  ...
  const hooks = createHooks<KumphouseHooks>()
  ...
  if (configFile)
    logger.info(`Creating Kumphouse ${configFile ? `using config from \`${configFile}\`` : ''}`)
  ...
  const ctx = {
    ...
    ws,
    provider,
  } as unknown as KumphouseContext
  ...
  const worker = await createKumphouseWorker(tasks)
  ...
  ctx.setCiContext = async () => {
    ...
    logger.debug(`Setting Kumphouse CI Context [Site: ${$site}]`)
    ...
    try {
      ...
      logger.error(`Failed to create output directory. Please check kumphouse has permissions to: ${resolvedConfig.outputPath}`, e)
    }
    ...
    try {
      ...
      logger.error(`Failed to create output directory. Please check kumphouse has permission to create files and folders in: ${resolvedConfig.outputPath}`, e)
    }
  ...
  ctx.setSiteUrl = async (url: string) => {
    ...
    logger.debug(`Setting Kumphouse Site URL [Site: ${site.toString()}]`)
  ...
  ctx.setServerContext = async ({ url, server, app }) => {
    ...
    logger.debug(`Setting Kumphouse Server Context [Server: ${$server}]`)
  ...
  ctx.start = async () => {
    logger.debug(`Starting Kumphouse [Server: ${provider?.name === 'ci' ? 'N/A' : ctx.runtimeSettings.clientUrl} Site: ${ctx.resolvedConfig.site} Debug: \`${ctx.resolvedConfig.debug}\`]`)
    ...
    if (provider?.name !== 'ci') {
      ...
      if (Number(latestTag.replace('v', '').replace('.', '')) > Number(version.replace('.', ''))) {
        title.push(...[
          ...
          colorize('gray', ` > ${colorize('underline', `npx kumphouse@^${latestTag} --site ${resolvedConfig.site}`)}`),
        ])
      }
      title.push(...[
        '',
