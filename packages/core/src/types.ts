...
/**
 * A normalised route, in the context of kumphouse, is a mapping between a URL path, it's definition and a unique id.
 */
 ...
 */
export type KumphouseTaskStatus = 'waiting' | 'in-progress' | 'completed' | 'failed' | 'ignore' | 'failed-retry'
...
export interface KumphouseRouteReport {
   ...
   */
  tasksTime?: Partial<Record<KumphouseTask, number>>
  /**
...
export type KumphouseTabs = 'overview' | LighthouseCategories

/**
 * Kumphouse's intelligent sampling relies on knowing which URLs map to which files in your project.
...
export interface ClientOptions {
   ...
   */
  columns: Record<KumphouseTabs, KumphouseColumn[]>
  /**
...
export interface ResolvedUserConfig {
   ...
   * By default, it attempts to load configuration from `kumphouse.config.ts`.
   *
   * You can set up multiple configuration files for different sites you want to scan.
   * For example:
   * - `staging-kumphouse.config.ts`
   * - `production-kumphouse.config.ts`
   */
  configFile?: string
  /**
  ...
  debug: boolean
  /**
   * Hooks to run to augment the behaviour of Kumphouse.
   */
  hooks?: NestedHooks<KumphouseHooks>
  /**
   * The URL path prefix for the client and API to run from.
   * Useful when you want to serve the application from an existing integrations server, you could use /__kumphouse
   *
   * @default ''
   */
  routerPrefix: string
...
export interface RuntimeSettings {
  ...
  /**
   * if the user has configured kumphouse using a configuration file, this is the link to it.
   */
  configFile?: string
  /**
...
export interface KumphouseWorkerStats {
  /**
...
export type PuppeteerTaskArgs = KumphouseRouteReport
export type PuppeteerTaskReturn = KumphouseRouteReport
export type PuppeteerTask = TaskFunction<PuppeteerTaskArgs, PuppeteerTaskReturn>
export type KumphousePuppeteerCluster = Cluster<PuppeteerTaskArgs, PuppeteerTaskReturn>
...
export interface KumphouseHooks {
  /**
   ...
   * @param response
   */
  'task-added': (path: string, response: KumphouseRouteReport) => HookResult
  /**
   * Fired when a task has started to work.
   * @param path
   * @param response
   */
  'task-started': (path: string, response: KumphouseRouteReport) => HookResult
  /**
   * Fired when a task has completed it's work.
   * @param path
   * @param response
   */
  'task-complete': (path: string, response: KumphouseRouteReport, taskName: string) => HookResult
  /**
 ...
 */
export interface KumphouseWorker {
  /**
   ...
   * @param report
   */
  requeueReport: (report: KumphouseRouteReport) => void
  /**
   ...
   */
  routeReports: Map<string, KumphouseRouteReport>
  /**
   * A simple array representation of the reports for easy iteration.
   */
  reports: () => KumphouseRouteReport[]
  /**
   * Find a report with the specified id.
   * @param id
   */
  findReport: (id: string) => KumphouseRouteReport | null
...
/**
 * The context is provided by the createKumphouse() or useKumphouse() functions.
 * It provides the central API to interact with the behaviour of Kumphouse..
 */
