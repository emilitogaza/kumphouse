...
import type { ResolvedUserConfig, KumphouseTabs, UserConfig } from './types'
...
export const resolveUserConfig: (userConfig: UserConfig) => Promise<ResolvedUserConfig> = async (userConfig) => {
  ...
  config.chrome = defu(config.chrome || {}, {
    ...
    downloadFallbackCacheDir: join(homedir(), '.kumphouse'),
  })
  ...
  if (config.client?.columns) {
    ...
    config.client.columns = pick(config.client.columns, ['overview', ...config.lighthouseOptions.onlyCategories as KumphouseTabs[]])
  }
