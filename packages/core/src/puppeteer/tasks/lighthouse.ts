...
import type { LighthouseReport, PuppeteerTask, KumphouseRouteReport } from '../../types'
...
export function normaliseLighthouseResult(route: KumphouseRouteReport, result: Result): LighthouseReport {
  const { resolvedConfig, runtimeSettings } = useKumphouse()
...
export const runLighthouseTask: PuppeteerTask = async (props) => {
  ...
  // just modify the url for the kumphouse request
