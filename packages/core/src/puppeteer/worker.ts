...
/**
 * The kumphouse worker is a wrapper for the puppeteer-cluster. It handles the queuing of the tasks with more control
 * over the clusters monitoring and queue management while providing a tight integration with kumphouse.
 *
 * @param tasks
 */
export async function createKumphouseWorker(tasks: Record<KumphouseTask, TaskFunction<PuppeteerTaskArgs, PuppeteerTaskReturn>>): Promise<KumphouseWorker> {
  const { hooks, resolvedConfig } = useKumphouse()
  const logger = useLogger()
  const progressBox = createProgressBox()
  const cluster = await launchPuppeteerCluster()

  const routeReports = new Map<string, KumphouseRouteReport>()
  ...
  const taskCompletionTimes = new Map<string, Record<KumphouseTask, number>>()

  // Track progress for live display
  const startTime = Date.now()
  let currentTaskInfo = ''

  const monitor: () => KumphouseWorkerStats = () => {
    const now = Date.now()
    ...
    if (donePercentage !== 0) {
      ...
      const taskDurations: Record<KumphouseTask, number[]> = {
        inspectHtmlTask: [],
        runLighthouseTask: [],
      }
      ...
      for (const [, completionTimes] of taskCompletionTimes) {
        for (const taskName of Object.keys(tasks) as KumphouseTask[]) {
  ...
  const queueRoute = (route: NormalisedRoute) => {
    ...
    const runTaskIndex = (idx = 0) => {
      ...
      const taskName = Object.keys(tasks)?.[idx] as KumphouseTask
      ...
      cluster
        ...
        .then((response) => {
          ...
          if (!taskCompletionTimes.has(id)) {
            taskCompletionTimes.set(id, {} as Record<KumphouseTask, number>)
          }
  ...
  const requeueReport = (report: KumphouseRouteReport) => {
