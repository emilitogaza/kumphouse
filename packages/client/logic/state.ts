...
export function openLighthouseReportIframeModal(report: KumphouseRouteReport, tab?: string) {
...
export const wsReports: Map<string, KumphouseRouteReport> = reactive(new Map<string, KumphouseRouteReport>())
...
const payloadReports = window.__kumphouse_payload?.reports
...
export const kumphouseReports = computed<KumphouseRouteReport[]>(() => {
  if (isStatic) {
    return window.__kumphouse_payload?.reports || []
  }
...
export const shouldShowWaitingState = computed<boolean>(() => {
  if (isStatic) {
    ...
    return !window.__kumphouse_payload?.reports?.length
  }
...
export const scanMeta = computed<ScanMeta | null>(() => {
  if (isStatic)
    return window.__kumphouse_payload?.scanMeta
...
export async function wsConnect() {
  try {
    ...
    try {
      ...
      const reports: KumphouseRouteReport[] = await response.json()
    ...
    console.warn('Failed to connect to Kumphouse server:', error)
    throw error // Re-throw to let caller handle it
  }
}

export const categoryScores = computed(() => {
  const reports = kumphouseReports.value || []
