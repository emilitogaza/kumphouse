...
import { kumphouseReports } from './state'
...
export const searchText = useStorage('kumphouse-search-text', '')
export const sorting = useStorage<Sorting>('kumphouse-sort', { key: groupRoutesKey, dir: 'asc' })
...
export const searchResults = computed<KumphouseRouteReport[]>(() => {
  let data = kumphouseReports.value || []
  ...
  if (searchText.value && data.length > 0) {
    ...
    data = fuse.search<KumphouseRouteReport>(searchText.value).map(i => i.item)
  }
  ...
  function statusToSortRank(s: KumphouseTaskStatus) {
    switch (s) {
      case 'completed':
        return 2
      case 'in-progress':
